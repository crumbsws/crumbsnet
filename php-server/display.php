...
if(isset($data['type']) && $data['type'] === 'posts')
{
    ...
    if(isset($data['parent'])){
        ...
        {
            $user = $data['user'];
            $sql = "SELECT * FROM paths WHERE parent='$parent' AND name='$user' ORDER BY date DESC";
        }
        else if(isset($data['club']))
        {
            $club = $data['club'];
            $sql = "SELECT * FROM paths WHERE parent='$parent' AND name IN (SELECT name FROM profile WHERE club='$club') ORDER BY date DESC";
        }
        else 
        {
            $sql = "SELECT * FROM paths WHERE parent='$parent' ORDER BY date DESC";
        }
    }
    else {
        $club = getClub($name);
        ...
        ELSE user_2 
        END AS friend FROM friends) ORDER BY date DESC";
    }
}
